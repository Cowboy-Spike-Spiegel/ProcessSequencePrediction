'''
this script takes as input the LSTM or RNN weights found by myTrain.py
change the path in line 178 of this script to point to the h5 file
with LSTM or RNN weights generated by myTrain.py
...
for x in timeseqs2:
    inner_mean = 0
    for y in x:
        difference = x[len(x)-1] - y
        inner_mean += difference
    inner_mean /= len(x)
    divisor3 += inner_mean
divisor3 /= len(timeseqs2)
...
# load model, set this to the model generated by myTrain.py
file = h5py.File('output_files/models/model_27-1.51.h5')
